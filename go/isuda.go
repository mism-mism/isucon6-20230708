...
func topHandler(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := db.Query(fmt.Sprintf(
		"SELECT id, keyword, description, author_id, updated_at, created_at FROM entry ORDER BY updated_at DESC LIMIT %d OFFSET %d",
...
func keywordByKeywordHandler(w http.ResponseWriter, r *http.Request) {
	...
	row := db.QueryRow(`SELECT id, keyword, description, author_id, updated_at, created_at FROM entry WHERE keyword = ?`, keyword)
	e := Entry{}
...
func htmlify(w http.ResponseWriter, r *http.Request, content string) string {
	...
	rows, err := db.Query(`
		SELECT id, keyword, description, author_id, updated_at, created_at FROM entry ORDER BY CHARACTER_LENGTH(keyword) DESC
	`)
	panicIf(err)
